---
title: "概要"
description: "さまざまな識別子を使用して会社、ファンド、暗号資産のロゴを検索・取得するための総合ガイド"
---
...
Benzinga Logos API では、企業、ファンド、暗号通貨の高品質なロゴを利用できます。シンボル、CIK 番号、ISIN コード、CUSIP、FIGI など、複数の識別子を使ってロゴを検索できます。
...
さまざまな識別子でロゴを検索します。ロゴのURL、証券情報、メタデータを返します。

<div id="logossync">
  ### `/logos/sync`
</div>

ページネーションに対応し、複数のロゴを一括で同期取得するためのエンドポイント。

<div id="search-filters">
  ## 検索フィルター
</div>

このAPIは、2つの主要なパラメータによる柔軟な検索をサポートしています。

* `search_keys` - 検索対象の識別子の値
* `search_keys_type` - 識別子の種類 (省略可能。デフォルトは `symbol`)

<div id="1-filter-by-symbol">
  ### 1. シンボルで絞り込む
</div>

株式のティッカーシンボルでロゴを検索します。
...
SEC の Central Index Key を使用して検索します。ゼロ埋めあり・なしの両方の値に対応しています。
...
ISIN (International Securities Identification Number) を使用して検索します。
...
<div id="4-filter-by-figi-share-class">
  ### 4. FIGI Share Class で絞り込む
</div>

Bloomberg の株式クラス用 Financial Instrument Global Identifier を使用して検索します。
...
<div id="6-filter-by-symbol-with-currency-country">
  ### 6. 通貨・国を指定したシンボルで絞り込む
</div>

区切り文字としてコロン (`:`) またはピリオド (`.`) を使用し、国コードまたは通貨コードを付けたシンボルを指定します。
...
取引所コードとシンボルを組み合わせて検索します。Bloomberg形式 (レガシー) とLSEG形式の両方に対応しています。

**Bloomberg形式のマッピング (レガシー) :**
...
* `mark_vector_composite_light` - ライトテーマのベクター複合マーク (SVG)
* `mark_vector_composite_dark` - ダークテーマのベクター複合マーク (SVG)
...
* `scale` - 画像のサイズを変更します (例: `100x100`、`300x300`) 
* `max_width` - 返される画像の最大幅
* `composite_radius` - 合成画像の角丸半径 (整数、範囲 `0-50`) 
* `composite_auto` - 合成画像を自動生成します (真偽値)
...
* `updated_since` - 更新日時でフィルタリング (ISO 8601 形式)

<div id="security-details">
  ### 証券の詳細
</div>

* `securities` - レスポンスに詳細な証券情報を含めます (boolean)
...
すべての Logos API エンドポイントで、APIキーによる認証が必要です。クエリパラメータとして token を指定してください。
...
1. **search&#95;keys&#95;type を明示的に指定する** - API のデフォルトは `symbol` ですが、タイプを明示的に指定することで、可読性とパフォーマンスが向上します
2. **必要なフィールドのみをリクエストする** - レスポンスサイズを最小限に抑え、パフォーマンスを向上させるため、必要なロゴタイプのみをリクエストしてください
3. **大規模なデータセットには bulk-sync を使用する** - 複数のロゴを取得する場合は、ページネーション付きの `/logos/bulk-sync` エンドポイントを使用してください
4. **レスポンスをキャッシュする** - ロゴ URL には有効期限のタイムスタンプが含まれているため、API 呼び出しを減らせるよう適切にキャッシュしてください
5. **エラーを適切に処理する** - レスポンス内の `ok` フィールドを必ず確認し、エラー時の処理を適切に実装してください

<div id="rate-limits">
  ## レートリミット
</div>

レートリミットの詳細については、ご利用のAPIサブスクリプションプランをご確認ください。アプリケーションでより高いレートリミットが必要な場合は、サポートまでお問い合わせください。

<div id="support">
  ## サポート
</div>

Logos API に関する追加のサポートが必要な場合やご質問がある場合は、Benzinga API サポートにお問い合わせいただくか、各エンドポイントの詳細なドキュメントを参照してください。