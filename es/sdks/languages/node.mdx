...
El SDK de TypeScript/JavaScript de Benzinga proporciona una interfaz modular basada en eventos para interactuar con las APIs de Benzinga. Escrito en TypeScript, el SDK funciona sin problemas tanto en navegadores como en entornos Node.js, y ofrece implementaciones mejoradas con almacenamiento en caché, comparación profunda y otras capacidades avanzadas.

<div id="key-features">
  ## Características principales
</div>

* **Compatibilidad con TypeScript** - Tipos e interfaces completas de TypeScript
* **Universal** - Funciona tanto en entornos de navegador como de Node.js
...
El SDK utiliza una arquitectura modular. Para empezar, instale el módulo principal de sesión:
...
El módulo `@benzinga/session` proporciona la base para autenticarse con las APIs de Benzinga. Todos los demás módulos dependen de este objeto Session.
...
Cada dominio de la API de Benzinga se empaqueta como un módulo de npm independiente. Esto permite:
...
El SDK utiliza un patrón basado en eventos para gestionar flujos y actualizaciones de datos en tiempo real:
...
* Ofrecer alternativas de respaldo sin conexión
...
* Reducir las renderizaciones en aplicaciones de UI

<div id="available-modules">
  ## Módulos disponibles
</div>

El SDK está organizado en módulos especializados para distintos dominios de la API:
...
* **`@benzinga/fundamentals`** - Fundamentales empresariales y datos financieros
...
* **`@benzinga/transcripts`** - Transcripciones de llamadas de resultados
* **`@benzinga/logos`** - Logotipos e identidad de marca
...
El SDK funciona en navegadores con empaquetadores como Webpack, Rollup o Vite:
...
Gestione los resultados paginados de forma eficiente:
...
<div id="1-reuse-session-objects">
  ### 1. Reutiliza los objetos de sesión
</div>

Crea una instancia de sesión y reutilízala en toda la aplicación:
...
Guarde las claves de la API de forma segura en variables de entorno:
...
Implemente límites de error en producción:
...
* **Paquete NPM**: [@benzinga/session](https://www.npmjs.com/package/@benzinga/session)
...
Para obtener documentación detallada sobre módulos específicos, consulta los archivos README de cada paquete:

* `@benzinga/session` - Autenticación y configuración principales
...
El SDK de JavaScript de Benzinga es de código abierto. ¡Se aceptan contribuciones! Visita el [repositorio de GitHub](https://github.com/Benzinga/benzinga-javascript-client) para obtener más información.